...
Live FinBot demo: OpenAI side-by-side comparison grounded in Colorado SNAP rules via the [Axiom rules engine](https://github.com/TheAxiomFoundation/axiom-rules-engine). No PolicyEngine, no model-recall numbers — every dollar amount comes from a real `axiom-rules-engine` compute.
...
- Tools shell out to the `axiom-rules-engine` Rust binary running against a compiled artifact built from `rulespec-us-co/policies/cdhs/snap/fy-2026-benefit-calculation.yaml`.
...
| Engine | `axiom-rules-engine` Rust binary, spawned per request via `child_process.spawn` |
| Rules content | `rulespec-us-co` (CO SNAP composition) + `rulespec-us` (federal imports) |
...
```bash
# 1. Clone axiom-rules-engine + rulespec-us + rulespec-us-co, build the binary, compile artifacts
...
The setup script clones the three Axiom repositories under `engine/` and runs `cargo build --release`. The engine binary lives at `engine/axiom-rules-engine/target/release/axiom-rules-engine`. Compiled artifacts land in `engine/artifacts/co-snap.compiled.json`.
...
- No fallback when `axiom-rules-engine` doesn't have a rule yet. The model says "axiom hasn't encoded that" instead of guessing.
...
`bun run engine:test` reproduces the verified base case: single applicant aged 60 in Colorado, $1,000/mo wages, $500 shelter cost, separate heating → **$298 monthly allotment, eligible**, matching `rulespec-us-co` test fixture exactly.
