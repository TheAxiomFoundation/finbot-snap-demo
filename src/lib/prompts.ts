  ...
  const digest = catalog.programs
    .map((p) => {
      ...
      const incomplete = p.acknowledged_incomplete.length
        ...
        : "";
      return `- ${p.slug} — ${p.display_name} (${grain}, defaults to ${defaultPeriodFor(p)}${incomplete})`;
    })
    .join("\n");
...
1. **Coverage** — the digest above is the authoritative program list; do NOT call list_programs just to confirm a program exists or to restate coverage. Call it only when you need \`search\` across encoded output names — at most once per question.
...
4. **lookup_value** — for questions about one specific encoded parameter or intermediate (an income limit, a maximum allotment, a deduction amount). Size-indexed values need the relevant *_size fact.
...
- Every dollar amount, eligibility verdict, threshold, or rate MUST come from compute, lookup_value, or fetch_citation. Do not estimate or recall from training. This includes "roughly" and "about" figures.
...
- Sanity-check zeros: if the primary output comes back $0 while an income-like slot (taxable_income, gross income, earned income) sat at its 0 default, that's your gap, not the answer. Fill the slot or tell the user which fact you need; never present that $0 as the result.
- Round dollars to whole numbers. Don't editorialize — no characterizing numbers as small, large, generous, or unfair.
- You may make up to 2 extra compute calls with varied pivotal facts to quantify "what could change this" (e.g. recompute with a member marked elderly, or with shelter costs included). Present those as deltas.
...
- If the applied report carries a WARNING about an auxiliary slot, your fact didn't reach the calculation: recompute once with the suggested on-path slot before answering.
